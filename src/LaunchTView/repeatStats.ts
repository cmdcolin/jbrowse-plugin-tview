...
export function alleleModes(copies: number[]): Tally[] {
  ...
  const peaks: Tally[] = []
  for (const t of tally(copies).sort(
    (a, b) => b.reads - a.reads || a.value - b.value,
  )) {
    const distance = (peak: Tally) => Math.abs(t.value - peak.value)
    const host = peaks
      .filter(
        peak =>
          distance(peak) <= Math.max(1, SHOULDER_SPREAD * peak.value) &&
